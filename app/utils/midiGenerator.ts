...
/** CRC-32 for ZIP (standard polynomial 0xEDB88320) */
...
export interface MidiElementInfo {
  element: string;
  category: string;
  role: string;
  intensity: number;
...
/** Generates all MIDI clips and downloads them as a single .zip file */
export function downloadAllMidisAsZip(
  elements: MidiElementInfo[],
  bpm: number,
  key?: string,
  genre?: string,
  trackName?: string
  ...
  const uniqueElements = elements.filter(el => {
    const k = el.element.toLowerCase();
  ...
  for (const el of uniqueElements) {
    ...
    const clip = generateMidiClip(ctx);
    const bytes = midiClipToBytes(clip);
    const safeName = el.element.replace(/[^a-zA-Z0-9_-]/g, '_');
    const folder = el.category.replace(/[^a-zA-Z0-9_-]/g, '_');
    files.push({
      name: `${folder}/${safeName}_${bpm}bpm_${clip.bars}bar.mid`,
      data: bytes,
    });
  }
