 ...
 */
async function fetchArtistReleases(request: Request, artist: string): Promise<ArtistRelease[]> {
  try {
    ...
    return data?.success && Array.isArray(data.albums) ? (data.albums as ArtistRelease[]) : [];
  } catch {
    return [];
  }
}
    ...
    const groups = await runWithConcurrency<typeof enabledArtists[number], FeedGroup | null>(
      ...
      async (artist) => {
        try {
          ...
          const releases = await fetchArtistReleases(request, artist.name);
          ...
          return { artist: artist.name, lastSeen, releases: releaseGroups, tracks };
        } catch (err) {
