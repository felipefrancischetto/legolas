...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Suporte para diferentes formatos de requisição
    let scrapeRequest: ScrapeRequest;
    
    if (body.url) {
      // Formato original do scrape-tracklist
      scrapeRequest = body;
    } else if (body.query) {
      // Formato de busca (tracklists-search)
      scrapeRequest = {
        url: `https://www.1001tracklists.com/search/?q=${encodeURIComponent(body.query)}`,
    ...
    const validationResult = scrapeRequestSchema.safeParse(scrapeRequest);
    if (!validationResult.success) {
      scrapingLogger.error(scrapeRequest.url || 'unknown', new Error('Validation failed'), 'api');
...
export async function GET(request: NextRequest) {
  ...
  try {
      ...
      return NextResponse.json({
        ...
        data: {
          ...
          cache: {
            ...
            persistentKeys: stats.persistentKeys,
    ...
    // Suporte para GET com query ou URL
    if (query || url) {
    ...
    console.error('Error in GET /api/tracklist/unified:', error);
    return NextResponse.json(
